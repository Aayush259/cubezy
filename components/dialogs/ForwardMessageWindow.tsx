            ...
            <div className="h-screen w-screen md:max-w-[90vw] md:h-[600px] md:w-[500px] md:max-h-[95vh] bg-black border border-gray-800 rounded-xl mx-auto overflow-hidden">
                <div className="w-full h-[85%] flex flex-col overflow-y-auto">
                    ...
                    {(!filteredConnections || filteredConnections.length === 0) ? (
                        ...
                        <label key={connection.userId._id} htmlFor={connection.userId._id} className="flex items-center justify-between gap-4 cursor-pointer border-b border-gray-800 px-2 py-4 md:px-6 md:py-6">
                            <div className="flex items-center gap-4">
                                <DP dp={connection.userId.dp} name={connection.userId.name} />
                                <p className="text-2xl">{connection.userId.name.split(" ")[0]}</p>
                            </div>

                            <input
                                type="checkbox"
                                id={connection.userId._id}
