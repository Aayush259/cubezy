...
import { formatDate, getRandomEmoji } from "@/lib/Funcs"
...
export default function Sidebar() {
    ...
    const sortedConnections = useMemo(() => {
        ...
        if (searchQuery !== "") {
            filteredConnections = filteredConnections.filter(connection => connection.userId.name.toLowerCase().includes(searchQuery.toLowerCase()))
        }
                    ...
                    return (
                        ...
                        >
                            <span className="flex items-center gap-4 flex-grow pr-7">
                                <DP dp={connection.userId.dp} name={connection.userId.name.split(" ")[0]} />

                                <span className="flex flex-col flex-grow justify-between items-start gap-[1px] lg:gap-[3px]">
                                    <span className="block">{connection.userId.name.split(" ")[0]}</span>
                            ...
                            <span className="flex items-center gap-2">
                                {
                                    lastMessage?._id && !lastMessage.isRead && lastMessage.senderId !== user?._id && (
