...
import { formatDate2 } from "@/lib/Funcs"
...
const Profile = () => {
    ...
    const { isProfileOpen, setIsProfileOpen, profileInfo } = useProfileContext()
            ...
            <div className="h-full w-full">
                {profileInfo && (
                    <div className="h-full w-full flex flex-col items-center gap-2 overflow-y-auto">
                        ...
                        {profileInfo._id === user?._id && profileInfo.connections && profileInfo.connections.length > 0 && <div className="w-full px-4 md:px-8 py-4 border-t border-b border-t-gray-800 border-b-gray-800 mt-3">
                            ...
                            <div className="shrink-0 grow-0 flex items-center gap-5 w-full overflow-x-auto">
                                {profileInfo.connections.map(connection => (
                                    <div
                                        ...
                                        key={connection.userId._id}
                                        onClick={() => handleConnectionClick(connection.userId._id)}
                                    >
                                        {connection.userId.dp ? (
                                            <span className="block w-[50px] h-[50px] rounded-full overflow-hidden">
                                                <Image
                                                    src={connection.userId.dp}
                                            ...
                                            <div className="h-[50px] w-[50px] flex items-center justify-center bg-blue-700 text-white text-xl rounded-full overflow-hidden">
                                                {connection.userId.name[0]}
                                            </div>
                                        )}

                                        <p className="max-w-[55px] overflow-ellipsis overflow-hidden">{connection.userId.name.split(" ")[0]}</p>
                                    </div>
                                ))}
                            </div>
                        </div>}
